...
function ViewDoctorTable() {
  ...
  const validateDoctorForm = () => {
    ...
    if (!currentDoctor.phoneNumber) errors.phoneNumber = "Phone number is required";
    ...
    <div className="mt-5">
      ...
      <TableContainer>
        <Table sx={{ minWidth: 800 }} aria-label="doctor table">
          ...
          <TableBody>
            {(rowsPerPage > 0
              ? filteredDoctors.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              : filteredDoctors
            ).map((doctor) => (
              <TableRow key={doctor.doctorId}>
                ...
                <TableCell>{new Date(doctor.joiningDate).toLocaleDateString()}</TableCell>
                <TableCell>
                  <Button
                    variant="contained"
                    color="secondary"
